jQuery(document).ready(function($) {
    ...
    $("#btn-rsvp-yes").click(function() {
        $("#btn-rsvp-yes").removeClass("btn--inactive").addClass("btn--active");
        $("#btn-rsvp-no").removeClass("btn--active").addClass("btn--inactive");
        $("#rsvp-option").val("Accept");
        $("#modal-rsvp").modal('show');
    });

    $('#btn-submit-rsvp-confirmation').click(function(){
      $("#rsvp-confirmation-modal").modal('hide');
      window.location.assign($('ul.inner-container__action-list>li:nth-child(2) > a.action:first-child').attr('href'));
      $("#modal-rsvp-success").modal('show');
    });

    $("#btn-rsvp-no").click(function() {
        $("#btn-rsvp-yes").removeClass("btn--active").addClass("btn--inactive");
        $("#btn-rsvp-no").removeClass("btn--inactive").addClass("btn--active");
    ...
    $("#btn-submit-rsvp").click(function(e) {
        ...
        $.post('/save_rsvp/' + window.location.search, $("#rsvp-form").serialize() + '&response=' + $("#rsvp-option").val())
            .done(function() {
                // Success Handling
        ...
        $("#event-date").each(function(){
            ...
            if (clientTimezone) {
                ...
                if (utcEvtDate.diff(moment.utc(), 'years') >= 2) {
                    ...
                    evtDateLink.text(clientTime.format('MMM Do h:mm a'));
                } else {
                    evtDateLink.text(clientTime.calendar());
                }
            } else {
                evtDateLink.text(utcEvtDate.format('MMM Do h:mm a'));
            }
        });
    }
